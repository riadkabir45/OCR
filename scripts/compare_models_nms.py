...
def predict_and_compare_with_nms(best_model_path, last_model_path, image_path, labels_dir,
    ...
    plt.suptitle(f'Model Comparison with Ground Truth and NMS (IoU={iou_threshold})\n{os.path.basename(image_path)}', fontsize=16)
    plt.tight_layout()
    plt.show()
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Compare YOLO models with NMS and ground truth')
    ...
    # Run comparison with NMS and ground truth
    results = predict_and_compare_with_nms(
